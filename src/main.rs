...
use archetype_lib::models::{
    Avatar, AvatarInfo, AvatarUri, DehydratedAvatarInfo, ResponseMessage, VersionInfo,
};
use archetype_lib::util::{extract_data_from_uri, get_version_code_from_string};
use archetype_lib::{
    delete_avatar_by_id_with_connection, get_all_avatars_with_connection,
    get_avatar_by_id_with_connection, get_file_as_base64_encoded_string, responders,
    update_avatar_by_id_with_connection,
};
use clap::Parser;
...
use rocket::{catchers, routes, Request};
...
) -> Result<RawJson<String>, responders::RequestError> {
    let connection = establish_connection(None);
...
async fn put_avatar_from_file(
    query_id: i32,
    upload_file: Data<'_>,
    _allowed_hosts: AllowedHosts,
    _file_size_guard: FileSizeChecker,
    limits: &Limits,
) -> Result<RawJson<String>, responders::RequestError> {
    ...
    let avt: Avatar = update_avatar_by_id_with_connection(
        &conn,
        query_id,
        b64_string,
...
async fn put_avatar_from_uri(
    query_id: i32,
    avatar_uri: rocket::serde::json::Json<AvatarUri>,
    _allowed_hosts: AllowedHosts,
    _file_size_guard: FileSizeChecker,
    limits: &Limits,
) -> Result<RawJson<String>, responders::RequestError> {
    ...
    let avt: Avatar = update_avatar_by_id_with_connection(&conn, query_id, data, mime_type)?;
    let info = AvatarInfo::from(&avt);
    let json_obj = serde_json::to_string(&info).unwrap();
    Ok(RawJson(json_obj))
}
...
fn get_all_avatars(
    ...
    let av_infos: Vec<DehydratedAvatarInfo> = avts
        .into_iter()
...
async fn upload_new_avatar_from_uri(
    avatar_uri: rocket::serde::json::Json<AvatarUri>,
    _file_size_guard: FileSizeChecker,
    _allowed_hosts: AllowedHosts,
) -> Result<RawJson<String>, responders::RequestError> {
...
async fn upload_new_avatar_from_file(
    upload_file: Data<'_>,
    _allowed_hosts: AllowedHosts,
    _file_size_guard: FileSizeChecker,
    limits: &Limits,
...
mod tests {
    ...
    use crate::{establish_connection, get_avatar_by_id, get_avatar_by_id_with_connection, Avatar};
    ...
    impl TestContext {
        ...
        fn get_last_created_avatar_id(&self) -> i32 {
            let last_av_id = self.avatar_ids_created.iter().rev().next().unwrap();
            *last_av_id
        }
    }
    //
    ...
    fn it_should_respond_with_404_if_attempting_to_get_an_unknown_id() {
        ...
        let avatar =
            get_avatar_by_id_with_connection(&connection, context.get_last_created_avatar_id());
